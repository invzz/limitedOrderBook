...
nlohmann::json OrderBook::getAsJson()
{
    std::shared_lock lock(bids_mtx);
    std::shared_lock lock2(asks_mtx);
    nlohmann::json   json;
    {
        ...
        for(const auto &[price, level] : bids)
            {
                if(level) buyOrdersJson.push_back(level->getAsJson());
            }
        json["bids"] = buyOrdersJson;

        // Serialize sell orders
        nlohmann::json sellOrdersJson = nlohmann::json::array();
        for(const auto &[price, level] : asks)
            {
                if(level) sellOrdersJson.push_back(level->getAsJson());
            }
        json["asks"] = sellOrdersJson;
    }
    return json;
}
